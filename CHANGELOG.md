...
- **stress-map 多进程客户端（`--procs=N`）+ 连接规模标定上探至 1000 连接（[benchmarks/ + docs]，
  单机容量天花板首次摸到硬信号）**：单进程 stream_select 客户端在 ~400 连接时自身饱和（一个 PHP 进程
  收 30 万+ 帧/s），测不到服务端天花板。改造：父进程 fork N 个 worker，各自独占连续 uid 段（1001 起
  按索引切分）与独立 select 循环，结果经临时 JSON 回传合并（计数求和 / 延迟直方图分桶相加 /
  窗口 = 各 worker 运行窗均值——建链因网关 bcrypt 串行而错峰，并集窗会低估 fps，故取均值）；
  **就绪栅栏**（建链完成落 ready 文件、等父进程 go 再统一开表）避免先建好的 worker 撞上后建者的
  登录洪峰；自测 +5 例（合并计数/均值窗/分桶相加/分位）。实测：400/600/800/1000 四档全量建链
  （auth=1000/1000），1000 连接时服务端 CPU 合计 331%（3 个承载 worker ≈ 单核饱和）——
  与「~200 连接/worker 近饱和」互相印证；每连接内存随扇出 306→709KB（容量规划上界）。
  1600 档受阻于 WSL 3.9GB 内存（非工具限制，归档说明）。同步**修正 §6.5 的 P99 归因**：
  多进程 400 档 P99 仍 1118ms，旧档「P99 劣化=单进程客户端饱和」被证伪，候选解释为服务端
  按连接字节配额/慢客户端软过滤丢弃低优先级 STATE 帧（待专项验证）。
