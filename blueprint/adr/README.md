...
| [ADR-028](ADR-028-Redis单点风险与HA路线.md) | Redis 单点风险与 HA 路线（第一期已交付；第二期见 ADR-031） | 已接受 |
| [ADR-029](ADR-029-频道间负载再均衡.md) | 频道间负载再均衡（提案态，触发条件未至） | 提议 |
| [ADR-030](ADR-030-协议v2一次切换与type词表压缩.md) | 协议 v2 一次切换 + type 词表压缩 + 清单协商地基 | 已接受 |
| [ADR-031](ADR-031-哨兵HA与连接自愈.md) | 哨兵 HA 与连接自愈（RedisConnector + 耐久屏障，兑现 ADR-028 第二期） | 已接受 |
